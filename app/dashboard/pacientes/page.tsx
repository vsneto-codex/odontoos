      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] bg-[#181C24] border border-white/10 rounded-2xl shadow-2xl flex flex-col">
        ...
        <div className="flex items-center gap-4 px-6 py-4 border-b border-white/5 flex-shrink-0">
          ...
          <button onClick={onFechar}
            className="text-white/30 hover:text-white transition-colors text-lg flex-shrink-0 w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/5"
        ...
        <div className="flex border-b border-white/5 flex-shrink-0 px-6">
          {(["dados", "historico"] as const).map((a) => (
            <button key={a} onClick={() => setAba(a)}
              className={`py-3 px-4 text-xs font-semibold uppercase tracking-wide transition-colors ${
                aba === a
        ...
        <div className="flex-1 overflow-y-auto px-6 py-5">
          {aba === "dados" && (
            <div className="space-y-4">
              ...
              <div className="text-white/20 text-xs uppercase tracking-widest font-semibold pb-1 border-b border-white/5">
                Informações básicas
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <input type="text" value={form.telefone ?? ""} onChange={(e) => setForm({ ...form, telefone: formatarTelefone(e.target.value) })} className={inputClass} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" value={form.cpf ?? ""} onChange={(e) => setForm({ ...form, cpf: formatarCPF(e.target.value) })} className={inputClass} />
                </div>
              </div>

              <div className="text-white/20 text-xs uppercase tracking-widest font-semibold pb-1 border-b border-white/5 pt-2">
                Endereço
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="col-span-2">
                  ...
                  <input type="text" value={form.endereco ?? ""} onChange={(e) => setForm({ ...form, endereco: e.target.value })} placeholder="Rua, número" className={inputClass} />
                </div>
                <div>
                ...
                <div>
                  ...
                  <input type="text" value={form.cep ?? ""} onChange={(e) => setForm({ ...form, cep: formatarCEP(e.target.value) })} placeholder="00000-000" className={inputClass} />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={form.convenio ?? ""} onChange={(e) => setForm({ ...form, convenio: e.target.value })} className={inputClass} />
                </div>
                <div>
                  <label className={labelClass}>Número</label>
                  <input type="text" value={form.numero_convenio ?? ""} onChange={(e) => setForm({ ...form, numero_convenio: e.target.value })} className={inputClass} />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea value={form.observacoes ?? ""} onChange={(e) => setForm({ ...form, observacoes: e.target.value })}
                  rows={3} placeholder="Anotações importantes sobre o paciente..."
                  className="w-full bg-[#13161C] border border-white/10 rounded-lg px-3 py-2 text-white text-sm placeholder-white/20 outline-none focus:border-[#4F8EF7] transition-colors resize-none" />
              </div>
          ...
          {aba === "historico" && (
            <div className="flex flex-col items-center justify-center h-48 text-center">
              ...
              <div className="text-white/40 text-sm">Histórico de consultas</div>
              <div className="text-white/20 text-xs mt-1">Em breve</div>
            </div>
          )}
        </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-white/5 flex-shrink-0">
          <button onClick={() => onExcluir(cliente.id, cliente.nome)}
            className="text-white/30 hover:text-red-400 text-sm transition-colors">
            Excluir paciente
          </button>
          <div className="flex gap-3">
            <button onClick={onFechar}
              className="h-9 px-5 bg-white/5 border border-white/10 text-white/60 text-sm rounded-lg hover:text-white transition-colors">
              Cancelar
            </button>
      ...
      {mostrarForm && (
        <div className="bg-[#1E2330] border border-white/10 rounded-xl p-5 mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
            ...
            <div>
              <label className={labelClass}>Telefone *</label>
              <input type="text" value={form.telefone} onChange={(e) => setForm({ ...form, telefone: formatarTelefone(e.target.value) })} placeholder="(15) 99999-0000" className={inputClass} />
            </div>
            <div>
              <label className={labelClass}>E-mail</label>
          ...
          <div className="flex gap-3">
            <button onClick={salvarNovoCliente} disabled={salvando || !form.nome || !form.telefone}
          ...
          <table className="w-full">
            ...
            <tbody>
              {clientesFiltrados.map((c, i) => (
                <tr key={c.id} onClick={() => setClienteSelecionado(c)}
                  className="border-b border-white/5 hover:bg-white/5 transition-colors cursor-pointer">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0"
                        style={{ background: `hsl(${(i * 47) % 360}, 70%, 20%)`, color: `hsl(${(i * 47) % 360}, 70%, 70%)` }}>
