...
function ModalPaciente({
  ...
  async function carregarHistorico() {
    ...
    console.log("Buscando histórico para:", JSON.stringify(cliente.nome));
    ...
    console.log("Consultas encontradas:", data?.length, data);
      ...
      <div className="relative w-full max-w-2xl max-h-[90vh] bg-[#181C24] border border-white/10 rounded-2xl shadow-2xl flex flex-col">
        ...
        <div className="flex items-center gap-4 px-6 py-4 border-b border-white/5 flex-shrink-0">
          ...
          <button onClick={onFechar}
            className="text-white/30 hover:text-white transition-colors w-8 h-8 flex items-center justify-center rounded-lg hover:bg-white/5"
          ...
          {(["dados", "historico"] as const).map((a) => (
            <button key={a} onClick={() => setAba(a)}
              className={`py-3 px-4 text-xs font-semibold uppercase tracking-wide transition-colors ${
                aba === a ? "text-[#4F8EF7] border-b-2 border-[#4F8EF7]" : "text-white/30 hover:text-white/60"
              }`}>
              ...
              {loadingConsultas ? (
                ...
                <div className="space-y-2">
                  {consultas.map((c) => {
                    ...
                    return (
                      <div key={c.id}
                        className="flex items-center gap-4 p-3 rounded-xl border border-white/5 hover:bg-white/5 transition-colors"
